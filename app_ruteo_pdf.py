# -*- coding: utf-8 -*-
"""
APP RUTEO PDF - Sin Excel, sin macros VBA.
...
from pdf_template_editor import PdfTemplateEditorWindow, PdfTemplateService
...
from app_update import CURRENT_VERSION, check_for_updates
...
def load_app_icon() -> QIcon:
    for icon_path in (resource_path("assets", "app_icon.ico"), resource_path("assets", "app_icon.png")):
        if icon_path.exists():
...
def configure_windows_taskbar_icon():
    ...
    try:
        ...
        ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID("RUTEO_PDF.Generador")
    except Exception:
        pass
...
EXCEL_TEMPLATE_COL_WIDTHS = [
    12.84, 18.36, 18.38, 21.96, 31.08, 19.92, 26.76, 33.00,
    24.00, 17.79, 36.12, 14.64, 39.84, 27.48, 28.56, 73.70,
    42.20, 51.55, 32.99, 28.32, 26.16, 26.52, 26.52, 54.02,
]
...
EXCEL_TEMPLATE_COLUMNS = [
    ...
    ("ACC/LECT", ["ACC/LECT", "ACC/LECTU", "ACCLECTU", "ACC LECT"], 20),
    ("HI", ["HI"], 21),
    ("HF", ["HF"], 22),
    ("PRECINTO Y OBSERVACION", ["PRECINTO Y OBSERVACION", "PRECINTO Y OBSERVACIÓN", "OBSERVACION", "OBSERVACIÓN"], 23),
]
...
def _build_reference_tables(style: ParagraphStyle, note_style: ParagraphStyle) -> Table:
    ...
    layout = Table(
        ...
        colWidths=[48, 160, 240, 150, 25, 110],
    )
...
def build_pdf_for_group(
    ...
    for row in rows:
        ...
        for label, source_idx in template_cols:
            ...
            body_row.append(_pdf_paragraph(_format_template_value(label, value), style))
    ...
    row_heights = [5.45] + [35.0] * len(rows)
    ...
    story = [
        ...
        KeepTogether([
            ...
            summary_table,
            Spacer(1, 12),
            _build_reference_tables(helper_style, note_style),
        ]),
    ]
    doc.build(story)
...
class VentanaRuteo(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"Aplicativo & persuasivas v{CURRENT_VERSION} - cierres y reaperturas")
    ...
    def _construir_menu(self):
        menu_importar = self.menuBar().addMenu("Importar")
        ...
        self.accion_acerca_de = QAction("Acerca del aplicativo CyR", self)
    ...
    def _construir_interfaz(self):
        ...
        titulo = QLabel("Aplicativo gestion cierres y reaperturas")
        ...
        self.combo_plantilla_pdf = self._crear_combo(["Normal", "Moderna"], editable=False)
    ...
    def show_busy_overlay(self, title: str, message: str):
        self.is_busy = True
        self._set_main_controls_enabled(False)
        if self.busy_overlay:
            self.busy_overlay.show_message(title, message)
    ...
    def check_app_updates_thread(self):
        ...
        self._run_thread(check_for_updates, self._finalizar_busqueda_actualizaciones)

    def _finalizar_busqueda_actualizaciones(self, info: Dict[str, object]):
        ...
        if bool(info.get("update_available")):
            ...
            if respuesta == QMessageBox.Yes and info.get("release_url"):
                QDesktopServices.openUrl(QUrl(str(info["release_url"])))
            return
    ...
    def show_about_dialog(self):
        QMessageBox.information(
            self,
            "Aplicacion sysaco breña",
            f"Aplicacion (C-y.R) v{CURRENT_VERSION}\n\n"
            "aplicativo cierres y persuasivas.\n"
            "Las actualizaciones estaran disponibles cuando el desarrrollar publique la nueva verion .",
        )
    ...
    def generate_pdfs_thread(self):
        ...
        if str(valores["pdf_template"]) == "Moderna":
            if not self.pdf_template_service.TemplateExists("Modern"):
                respuesta = QMessageBox.question(
                    self,
                    "Plantilla moderna",
            ...
            QMessageBox.information(
                self,
                "Plantilla moderna preparada",
                "La plantilla moderna ya está guardada. En esta etapa el generador masivo conserva la plantilla normal; la conexión completa se agregará en la siguiente fase.",
            )
            return
